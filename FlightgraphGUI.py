...
with st.sidebar:
    ...
    # Load data button
    if st.button("📥 Load Flight Data", use_container_width=True):
        with st.spinner("Loading flight data..."):
            st.session_state.data = get_flight_data()
            if st.session_state.data is not None:
                st.session_state.data_loaded = True
                st.success(f"✅ Loaded {len(st.session_state.data)} flight records")
            else:
                st.error("❌ Failed to load flight data")
    
    if st.session_state.data_loaded and st.session_state.data is not None:
        st.info(f"📊 Data loaded: {len(st.session_state.data)} records")
    ...
    st.markdown(
        ...
        ### How to use:
        1. Load flight data
        2. Select search type (Direct/Indirect)
...
if not st.session_state.data_loaded or st.session_state.data is None:
    st.warning("⚠️ Please load flight data first using the button in the sidebar!")
else:
